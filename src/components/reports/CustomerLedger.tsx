...
// // SOLUTION: Move search component OUTSIDE parent to prevent recreation
// const CustomerLedgerSearchIsolated = React.memo<{
//   onFinalSearch: (value: string) => void;
// }>(({ onFinalSearch }) => {
//   const [localSearch, setLocalSearch] = useState(() => globalSearchValue || '');
//   const debouncedSearch = useDebounce(localSearch, 500);

//   // Keep global in sync to prevent state loss
//   useEffect(() => {
//     globalSearchValue = localSearch;
//   }, [localSearch]);

//   // DEBUG: Check if callback changes
//   const callbackRef = useRef(onFinalSearch);
//   if (callbackRef.current !== onFinalSearch) {
//     console.log('⚠️ onFinalSearch callback changed! This will cause recreation.');
//     callbackRef.current = onFinalSearch;
//   }

//   // Only notify parent when search is finalized (debounced)
//   useEffect(() => {
//     onFinalSearch(debouncedSearch);
//   }, [debouncedSearch, onFinalSearch]);

//   const handleInputChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
//     console.log('🎯 Isolated input change:', e.target.value);
//     const newValue = e.target.value;
//     setLocalSearch(newValue);
//     globalSearchValue = newValue; // Keep global in sync immediately
//   }, []);

//   console.log('🔍 Isolated Search render:', { localSearch, debouncedSearch });
...
//         autoComplete="off"
//         spellCheck={false}
//         onFocus={() => console.log('🎯 Isolated Input focused')}
//         onBlur={() => console.log('🎯 Isolated Input blurred')}
//       />
//       {localSearch.length > 0 && localSearch !== debouncedSearch && (
...
// // Custom hook for client-side filtering without parent re-renders
// const useClientSideFilter = (transactions: any[], filters: any) => {
...
//       filtered = filtered.filter((tx: any) => tx.date >= filters.from_date);
//     }

//     if (filters.to_date) {
//       filtered = filtered.filter((tx: any) => tx.date <= filters.to_date);
//     }

//     // Apply search filter using global search value
//     const currentSearchValue = globalSearchValue;
//     if (currentSearchValue && currentSearchValue.trim().length > 0) {
...
//   }, [transactions, filters.type, filters.from_date, filters.to_date]);

//   // Set up interval to check for search changes without triggering parent re-renders
//   useEffect(() => {
//     const interval = setInterval(() => {
//       // Re-run filtering if global search changed
//       let filtered = [...transactions];
...
// // GLOBAL: Create callback outside component to ensure it never changes
// let globalSearchCallback: ((value: string) => void) | null = null;
// let globalSearchValue = '';

// const CustomerLedger: React.FC = () => {
//   // DEBUG: Track component re-renders with stack trace
//   console.log('🔄 CustomerLedger component render');
//   console.trace('🔍 CustomerLedger render stack trace');
...
//   // 🚀 BACK TO BASICS: Copy exact working CustomerList pattern
//   const [searchQuery, setSearchQuery] = useState('');
//   const debouncedSearchQuery = useDebounce(searchQuery, 500);

//   // Simple search handler - EXACTLY like CustomerList
//   const handleSearch = useCallback((value: string) => {
//     setSearchQuery(value);
//   }, []);

//   // Simple filtering exactly like CustomerList
...
//     return filtered;
//   }, [customerTransactions, filters.type, filters.from_date, filters.to_date, debouncedSearchQuery]);
...
//   // FIXED: Stable filter change handler - prevents re-renders
...
//         search: '',
//         limit: 50000, // Increased limit to get ALL customers (24k+)
//         offset: 0,
//         includeBalance: true
//       });
...
//     console.log('🧹 clearFilters called - this should only happen on Clear button click');
//     setSearchQuery(''); // Clear the search query like CustomerList
//     setFilters({
//       from_date: '',
//       to_date: '',
//       type: '',
...
//             {/* 🚀 SIMPLE SEARCH: Exact copy of working CustomerList */}
//             <div className="relative flex-1 min-w-0">
//               <Search className="h-5 w-5 absolute left-3 top-2.5 text-gray-400" />
//               <input
...
//             {(filters.from_date || filters.to_date || filters.type || searchQuery) && (
//               <button
//                 onClick={clearFilters}
//                 className="px-4 py-2 text-gray-600 hover:text-gray-800 text-sm whitespace-nowrap"
//               >
//                 Clear All
//               </button>
//             )}
...
//                   {filters.type || filters.from_date || filters.to_date || filters.search
