...
const DailyLedger: React.FC = () => {
  ...
  // � BULLETPROOF FIX: Use refs to ensure handlers always have current values
  ...
  useEffect(() => {
    ...
    try {
      if (typeof window !== 'undefined') {
        // 🚨 CRITICAL FIX: Use imported eventBus, not window.eventBus
        console.log('🚨 CRITICAL DEBUG: EventBus instance:', eventBus);
        console.log('🚨 CRITICAL DEBUG: EventBus methods:', Object.keys(eventBus));
        console.log('🚨 CRITICAL DEBUG: BUSINESS_EVENTS:', BUSINESS_EVENTS);

        // 🚨 DEEP DEBUG: Add universal event listener to catch ALL events
        const originalEmit = eventBus.emit;
        eventBus.emit = function (event: string, data: any) {
          console.log('🚨 UNIVERSAL EVENT CAUGHT:', event, data);
          if (event.includes('payment') || event.includes('PAYMENT')) {
            console.log('🚨 PAYMENT-RELATED EVENT DETECTED:', event, data);
          }
        ...
        if (eventBus && eventBus.on) {
          ...
          // Subscribe to relevant events for comprehensive real-time updates
            ...
            const today = formatDateForDatabase();
            console.log('🚨 BULLETPROOF: Today formatted:', today);
            console.log('🚨 BULLETPROOF: Date comparison - selectedDate === today?', selectedDate === today);
          ...
          // 🚨 CRITICAL FIX: Use actual event string, not constant name
          eventBus.on(BUSINESS_EVENTS.PAYMENT_RECORDED, handlePaymentReceived); // payment:recorded
          eventBus.on('payment:recorded', handlePaymentReceived); // Backup for safety
          eventBus.on('PAYMENT_RECORDED', handlePaymentReceived); // Legacy support
  ...
  }, [selectedDate, selectedCustomerId, selectedPaymentChannels, handleDailyLedgerUpdate, handleInvoiceCreated, handlePaymentReceived]);
    ...
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            onClick={() => {
              console.log('🚨 DEBUG: Testing event emission');
              eventBus.emit(BUSINESS_EVENTS.PAYMENT_RECORDED, {
                ...
                date: formatDateForDatabase(),
                paymentMethod: 'Debug',
                paymentChannel: 'Debug Channel',
                invoiceId: 999999,
                type: 'debug_test'
              });
            }}
            className="p-2 border rounded-lg hover:bg-red-100 bg-red-50 border-red-300"
